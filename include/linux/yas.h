...
struct yas_mag_driver {
    ...
    int (*get_transform_matrix)(int *matrix);
    int (*set_transform_matrix)(int *matrix);
#endif
    int (*get_enable)(void);
...
#include "manip.h"
#else
#include "ms3axesmscalib.h"
#endif

#ifdef YAS_MAG_SUPPORT_ELLIPSOID
typedef struct {
    msfloat range_threshold[3];
    msfloat distortion[3];
} ManipCalibThreshold;
#endif

struct yas_calibration {
    int initialized;
    struct yas_mag_calibration_callback callback;
#ifdef YAS_MAG_SUPPORT_ELLIPSOID
    Manip calib_module[YAS_MAGCALIB_SHAPE_NUM];
    ManipResult result;
    ManipCalibThreshold settings[3];
    int accuracy[YAS_MAGCALIB_SHAPE_NUM];
    int mode;
    int num_samples;
    int num_range_detector;
#else
    Ms3AxesMsCalib calib_module[YAS_MAGCALIB_SHAPE_NUM];
    Ms3AxesMsCalibResult result;
    Ms3AxesMsCalibThreshold settings[3];
#endif
    int shape;
    int spread;
    int variation;
...
struct yas_mag_calibration {
    int (*init)(struct yas_calibration *t);
    int (*term)(struct yas_calibration *t);
    int (*update)(struct yas_calibration *t, struct yas_vector *mag,
            struct yas_mag_calibration_result *result);
    int (*get_accuracy)(struct yas_calibration *t);
    int (*set_accuracy)(struct yas_calibration *t, int accuracy);
    int (*get_offset)(struct yas_calibration *t, struct yas_vector *offset);
    int (*set_offset)(struct yas_calibration *t, struct yas_vector *offset);
    int (*get_shape)(struct yas_calibration *t);
    int (*set_shape)(struct yas_calibration *t, int shape);
    int (*get_threshold)(struct yas_calibration *t,
            struct yas_mag_calibration_threshold *threshold);
    int (*set_threshold)(struct yas_calibration *t,
            struct yas_mag_calibration_threshold *threshold);
#ifdef YAS_MAG_SUPPORT_ELLIPSOID
    int (*get_mode)(struct yas_calibration *t);
    int (*set_mode)(struct yas_calibration *t, int mode);
    int (*get_max_sample)(struct yas_calibration *t);
    int (*set_max_sample)(struct yas_calibration *t, int num_samples);
    int (*get_transform_matrix)(struct yas_calibration *t, int32_t *matrix);
#endif
    struct yas_mag_calibration_callback callback;
};
#else
struct yas_mag_calibration {
    ...
    int (*get_transform_matrix)(int32_t *matrix);
#endif
    struct yas_mag_calibration_callback callback;
};
#endif
...
#ifdef YAS_MAGCALIB_INSTANTIATE
int yas_mag_calibration_init(struct yas_calibration *t, struct yas_mag_calibration *f);
#else
