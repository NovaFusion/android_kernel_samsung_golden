...
#define YAS_BMA150_DEFAULT_POSITION                                            0
...
static int yas_bma150_write_reg_byte(int, int);
...
static int yas_bma150_read_reg(unsigned char adr, unsigned char *buf,
			       unsigned char len)
{
...
static int yas_bma150_write_reg(unsigned char adr, unsigned char *buf,
unsigned char len)
{
	struct yas_acc_driver_callback *cbk = &pcb->callback;
	int err;
...
static int yas_bma150_write_reg_byte(int adr, int val)
{
	return yas_bma150_write_reg((unsigned char)adr,
				    (unsigned char *)&val, 1);
}
...
#define yas_bma150_update_bits(r, v) \
	yas_bma150_write_reg_byte(r##_REG, \
				  ((yas_bma150_read_reg_byte(r##_REG) & \
				    ~r##_MASK) | ((v) << r##_SHIFT)))
...
static int yas_bma150_init(void)
{
	...
	/* Reset chip */
	yas_bma150_power_up();
	yas_bma150_write_reg_byte(YAS_BMA150_SOFT_RESET_REG,
				  YAS_BMA150_SOFT_RESET_MASK);
...
static int yas_bma150_set_delay(int delay)
{
	...
	for (i = 1; i < (int)(sizeof(yas_bma150_odr_tbl) /
			      sizeof(struct yas_bma150_odr)) &&
...
static int yas_bma150_set_enable(int enable)
{
	...
	if (yas_bma150_ischg_enable(enable)) {
		if (enable) {
			...
			yas_bma150_write_reg_byte(YAS_BMA150_SOFT_RESET_REG,
						  YAS_BMA150_SOFT_RESET_MASK);
			/* wait 10us after soft_reset
			   to start I2C transaction */
			yas_bma150_msleep(1);
			/* Set axes range*/
			yas_bma150_update_bits(YAS_BMA150_RANGE,
					       YAS_BMA150_RANGE_2G);
...
static int yas_bma150_data_filter(int data[], int raw[],
...
#if 0
static void yas_acc_get_data(uint8_t *buf, int *raw)
{
	int i;
	for (i = 0; i < 3; i++)
		raw[i] = ((int16_t)((buf[i*2+1] << 8)) |
...
static int yas_acc_data_resolution(int val)
{
	return val *= YAS_BMA150_GRAVITY_EARTH / YAS_BMA150_RESOLUTION;
}
#endif
...
static int yas_set_offset(struct yas_vector *offset)
{
	...
	if (offset == NULL ||
	    offset->v[0] < YAS_BMA150_ABSMIN_2G ||
	    YAS_BMA150_ABSMAX_2G < offset->v[0] ||
	    offset->v[1] < YAS_BMA150_ABSMIN_2G ||
	    YAS_BMA150_ABSMAX_2G < offset->v[1] ||
	    offset->v[2] < YAS_BMA150_ABSMIN_2G ||
	    YAS_BMA150_ABSMAX_2G < offset->v[2])
		return YAS_ERROR_ARG;
