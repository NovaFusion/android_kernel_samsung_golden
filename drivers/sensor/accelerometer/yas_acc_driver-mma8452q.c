...
static int yas_mma8452q_read_reg(unsigned char, unsigned char *, unsigned char);
static int yas_mma8452q_write_reg(unsigned char, unsigned char *,
				  unsigned char);
...
static int yas_mma8452q_measure(int *, int *);
...
static int yas_mma8452q_read_reg(unsigned char adr, unsigned char *buf,
				 unsigned char len)
{
...
static int yas_mma8452q_write_reg(unsigned char adr, unsigned char *buf,
				  unsigned char len)
{
	struct yas_acc_driver_callback *cbk = &pcb->callback;
	int err;
...
static int yas_mma8452q_set_delay(int delay)
{
	...
	for (i = 1; i < (int)(sizeof(yas_mma8452q_odr_tbl) /
			      sizeof(struct yas_mma8452q_odr)) &&
...
static int yas_mma8452q_data_filter(int data[], int raw[],
				    struct yas_mma8452q_acceleration *accel)
{
...
static int yas_mma8452q_measure(int *out_data, int *out_raw)
{
	struct yas_mma8452q_acceleration accel;
	unsigned char buf[6];
	...
	for (i = 0; i < 3; i++) {
		...
		for (j = 0; j < 3; j++)
			data[i] += raw[j] *
				yas_mma8452q_position_map[pos][i][j];
		/* normalization */
		data[i] *= (YAS_MMA8452Q_GRAVITY_EARTH /
			    YAS_MMA8452Q_RESOLUTION);
	}

	yas_mma8452q_data_filter(data, raw, &accel);
...
static int yas_set_offset(struct yas_vector *offset)
{
	...
	if (offset == NULL ||
	    offset->v[0] < YAS_MMA8452Q_ABSMIN_2G ||
	    YAS_MMA8452Q_ABSMAX_2G < offset->v[0] ||
	    offset->v[1] < YAS_MMA8452Q_ABSMIN_2G ||
	    YAS_MMA8452Q_ABSMAX_2G < offset->v[1] ||
	    offset->v[2] < YAS_MMA8452Q_ABSMIN_2G ||
	    YAS_MMA8452Q_ABSMAX_2G < offset->v[2])
		return YAS_ERROR_ARG;
...
static int yas_set_filter(struct yas_acc_filter *filter)
{
	...
	if (filter == NULL || filter->threshold < 0 ||
	    filter->threshold > YAS_MMA8452Q_ABSMAX_2G)
		return YAS_ERROR_ARG;
...
static int yas_measure(struct yas_acc_data *data)
{
	...
	yas_mma8452q_lock();
	err = yas_mma8452q_measure(data->xyz.v, data->raw.v);
	yas_mma8452q_unlock();
