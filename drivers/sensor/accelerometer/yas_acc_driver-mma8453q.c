...
static int yas_mma8453q_write_reg(unsigned char, unsigned char *,
				  unsigned char);
static int yas_mma8453q_read_reg_byte(unsigned char);
static int yas_mma8453q_write_reg_byte(int, int);
...
static int yas_mma8453q_read_reg(unsigned char adr, unsigned char *buf,
				 unsigned char len)
{
...
static int yas_mma8453q_write_reg(unsigned char adr, unsigned char *buf,
				  unsigned char len)
{
	struct yas_acc_driver_callback *cbk = &pcb->callback;
	int err;
...
static int yas_mma8453q_write_reg_byte(int adr, int val)
{
	return yas_mma8453q_write_reg((unsigned char)adr,
				      (unsigned char *)&val, 1);
}
...
#define yas_mma8453q_update_bits(r, v) \
	yas_mma8453q_write_reg_byte(r##_REG, \
				((yas_mma8453q_read_reg_byte(r##_REG) & \
				  ~r##_MASK) | ((v) << r##_SHIFT)))
...
static int yas_mma8453q_set_delay(int delay)
{
	...
	for (i = 1; i < (int)(sizeof(yas_mma8453q_odr_tbl) /
			      sizeof(struct yas_mma8453q_odr)) &&
...
static int yas_mma8453q_data_filter(int data[], int raw[],
...
static int yas_mma8453q_measure(int *out_data, int *out_raw)
{
	...
	for (i = 0; i < 3; i++) {
		...
		for (j = 0; j < 3; j++)
			data[i] += raw[j] *
				yas_mma8453q_position_map[pos][i][j];
		/* normalization */
		data[i] *= (YAS_MMA8453Q_GRAVITY_EARTH /
			    YAS_MMA8453Q_RESOLUTION);
	}
...
static int yas_set_offset(struct yas_vector *offset)
{
	...
	if (offset == NULL ||
	    offset->v[0] < YAS_MMA8453Q_ABSMIN_2G ||
	    YAS_MMA8453Q_ABSMAX_2G < offset->v[0] ||
	    offset->v[1] < YAS_MMA8453Q_ABSMIN_2G ||
	    YAS_MMA8453Q_ABSMAX_2G < offset->v[1] ||
	    offset->v[2] < YAS_MMA8453Q_ABSMIN_2G ||
	    YAS_MMA8453Q_ABSMAX_2G < offset->v[2])
		return YAS_ERROR_ARG;
...
static int yas_set_filter(struct yas_acc_filter *filter)
{
	...
	if (filter == NULL || filter->threshold < 0 ||
	    filter->threshold > YAS_MMA8453Q_ABSMAX_2G)
		return YAS_ERROR_ARG;
