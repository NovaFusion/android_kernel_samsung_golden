...
#define YAS_KXTF9_WHO_AM_I                                                  0x01
...
static int yas_kxtf9_write_reg_byte(int, int);
...
static int yas_kxtf9_read_reg(unsigned char adr, unsigned char *buf,
			      unsigned char len)
{
...
static int yas_kxtf9_write_reg(unsigned char adr, unsigned char *buf,
			       unsigned char len)
{
	struct yas_acc_driver_callback *cbk = &pcb->callback;
	int err;
...
static int yas_kxtf9_write_reg_byte(int adr, int val)
{
	return yas_kxtf9_write_reg((unsigned char)adr,
				   (unsigned char *)&val, 1);
}
...
#define yas_kxtf9_update_bits(r, v) \
	yas_kxtf9_write_reg_byte(r##_REG,				\
				 ((yas_kxtf9_read_reg_byte(r##_REG) & \
				   ~r##_MASK) | ((v) << r##_SHIFT)))
...
static int yas_kxtf9_set_delay(int delay)
{
	...
	for (i = 1; i < (int)(sizeof(yas_kxtf9_odr_tbl) /
			      sizeof(struct yas_kxtf9_odr)) && \
...
static int yas_kxtf9_set_enable(int enable)
{
	...
	if (yas_kxtf9_ischg_enable(enable)) {
		if (enable) {
			...
			yas_kxtf9_msleep(1);
			/* Set data output range 12bit */
			yas_kxtf9_update_bits(YAS_KXTF9_DATA_RANGE,
					      YAS_KXTF9_DATA_RANGE_12);
			/* Set axes range*/
			yas_kxtf9_update_bits(YAS_KXTF9_GSEL,
					      YAS_KXTF9_GSEL_2G);
			/* Set enable function */
			yas_kxtf9_update_bits(YAS_KXTF9_ENABLE_FUNC,
					      YAS_KXTF9_ENABLE_FUNC_TDTE |
...
static int yas_kxtf9_data_filter(int data[], int raw[],
...
static int yas_set_offset(struct yas_vector *offset)
{
	...
	if (offset == NULL ||
	    offset->v[0] < YAS_KXTF9_ABSMIN_2G ||
	    YAS_KXTF9_ABSMAX_2G < offset->v[0] ||
	    offset->v[1] < YAS_KXTF9_ABSMIN_2G ||
	    YAS_KXTF9_ABSMAX_2G < offset->v[1] ||
	    offset->v[2] < YAS_KXTF9_ABSMIN_2G ||
	    YAS_KXTF9_ABSMAX_2G < offset->v[2])
		return YAS_ERROR_ARG;
...
static int yas_set_filter(struct yas_acc_filter *filter)
{
	...
	if (filter == NULL || filter->threshold < 0 ||
	    filter->threshold > YAS_KXTF9_ABSMAX_2G)
		return YAS_ERROR_ARG;
