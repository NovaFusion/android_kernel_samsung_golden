...
#if defined(CONFIG_MACH_SEC_SKOMER)
	if (atomic_read(&flgCalSet)) {
		accel_open_calibration();
		atomic_set(&flgCalSet, 0);
		pr_info("%s: flgcalset\n", __func__);
	}
#endif
#ifdef CONFIG_BMA254_SMART_ALERT
							...
							size_t count)
{
	...
	pr_info("%s: value = %d, onoff = %d, state =%d\n",
		__func__, value,
