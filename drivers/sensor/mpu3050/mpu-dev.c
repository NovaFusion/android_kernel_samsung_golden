...
int read_accel_raw_xyz(struct acc_data *acc)
{
	...
	{
			if(sensor_i2c_read(this_client->adapter, 0x08, 0x02, 6, acc_data) != 0) return -1;
	}
...
*/
	...
	for(j=0;j<3;j++) {	
	     ...
	     acc_bma[j]=(BMA222_GET_BITSLICE(acc_data[2*j+1],BMA222_ACC_MSB)<<(BMA222_ACC_LSB__LEN));
	}
...
static DEVICE_ATTR(gyro_get_temp, S_IRUGO | S_IWUSR | S_IXOTH,
		mpu3050_get_temp, NULL);
static DEVICE_ATTR(gyro_selftest, S_IRUGO | S_IWUSR | S_IXOTH,
		mpu3050_self_test, NULL);
static struct device_attribute *gyro_sensor_attrs[] = {
	&dev_attr_gyro_power_on,
	&dev_attr_gyro_get_temp,
