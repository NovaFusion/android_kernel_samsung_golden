...
struct mpu6050_power_data {
	struct regulator *regulator_vdd;
	struct regulator *regulator_vio;
};

static struct mpu6050_power_data mpu6050_power;
				...
				unsigned short length, unsigned char *data)
{
	...
	while (bytes_read < length) {
		...
		result =
		    mpu6050_i2c_read_reg(i2c_client, MPUREG_FIFO_R_W, this_len,
					 &data[bytes_read]);
		if (result)
			return result;
		bytes_read += this_len;
...
static void mpu6050_input_report_accel_xyz(struct mpu6050_input_data *data)
{
	...
	result =
	    mpu6050_i2c_read_reg(data->client, MPUREG_ACCEL_XOUT_H, 6, regs);
...
static int mpu6050_input_set_fsr(struct mpu6050_input_data *data, int fsr)
{
	...
	result =
	    mpu6050_i2c_read_reg(data->client, MPUREG_ACCEL_CONFIG, 1, &reg);
	if (result) {
...
#ifdef CONFIG_INPUT_MPU6050_POLLING
	int enable_old = atomic_read(&data->accel_enable);
#endif
					       ...
					       const char *buf, size_t count)
{
	...
	unsigned long value = 0;
#ifdef CONFIG_INPUT_MPU6050_POLLING
	int enable_old = atomic_read(&data->gyro_enable);
#endif
					 ...
					 const struct i2c_device_id *id)
{
	...
	mpu6050_power.regulator_vdd = mpu6050_power.regulator_vio = NULL;
	mpu6050_power.regulator_vdd = regulator_get(&client->dev, "vdd_mpu");
	if (IS_ERR(mpu6050_power.regulator_vdd)) {
		ret = PTR_ERR(mpu6050_power.regulator_vdd);
		pr_err("%s: failed to get accsns_i2c_vdd %d\n", __func__, ret);
		goto err_setup_regulator;
	}
	if (system_rev >= GOLDEN_R0_2){
	mpu6050_power.regulator_vio = regulator_get(&client->dev, "vio_inv");
	if (IS_ERR(mpu6050_power.regulator_vio)) {
		ret = PTR_ERR(mpu6050_power.regulator_vio);
		pr_err("%s: failed to get accsns_i2c_vio %d\n", __func__, ret);
		goto err_setup_regulator;
		}
	} else {
	mpu6050_power.regulator_vio = regulator_get(&client->dev, "vio_mpu");
	if (IS_ERR(mpu6050_power.regulator_vio)) {
		ret = PTR_ERR(mpu6050_power.regulator_vio);
		pr_err("%s: failed to get accsns_i2c_vio %d\n", __func__, ret);
		goto err_setup_regulator;
	...
	regulator_enable(mpu6050_power.regulator_vdd);

	mdelay(15);

	regulator_enable(mpu6050_power.regulator_vio);
	...
	return 0;
err_setup_regulator:
	if (mpu6050_power.regulator_vdd) {
		regulator_disable(mpu6050_power.regulator_vdd);
		regulator_put(mpu6050_power.regulator_vdd);
	}
	if (mpu6050_power.regulator_vio) {
		regulator_disable(mpu6050_power.regulator_vio);
		regulator_put(mpu6050_power.regulator_vio);
	}
