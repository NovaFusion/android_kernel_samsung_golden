...
static int pn547_dev_chip_config(enum pn547_state state, struct pn547_dev *dev)
{
    ...
    case PN547_STATE_OFF:
        gpio_set_value_cansleep(dev->pdata->firm_gpio, 0);
    ...
    case PN547_STATE_FWDL:
        gpio_set_value_cansleep(dev->pdata->ven_gpio, 1);
        gpio_set_value_cansleep(dev->pdata->firm_gpio, 1);
        msleep(PN547_WAIT_VENENABLE);
        gpio_set_value_cansleep(dev->pdata->ven_gpio, 0);
        msleep(PN547_WAIT_VENENABLE);
        gpio_set_value_cansleep(dev->pdata->ven_gpio, 1);
            ...
            const struct i2c_device_id *id)
{
	...
	/*ret = request_irq(client->irq, pn547_dev_irq_handler,
			  IRQF_TRIGGER_RISING, client->name, dev);*/
    if (IS_ERR_VALUE(ret)) {
    ...
    dev->nfc_regulator = NULL;
    dev->nfc_regulator = regulator_get(&client->dev, "v_sysclk4_nfc");
    if(IS_ERR(dev->nfc_regulator)) {
        dev_err(&client->dev, "%s: regulator get failed\n", __func__);
        goto probe_regulator_get;
    }

    /* Enable the regulator */
    if(regulator_enable(dev->nfc_regulator)) {
        dev_err(&client->dev, "%s: regulator enable failed\n", __func__);
        goto probe_regulator_enable;
    }
...
probe_regulator_enable:
    regulator_disable(dev->nfc_regulator);
    regulator_put(dev->nfc_regulator);
probe_regulator_get:
probe_irq:
...
static int pn547_remove(struct i2c_client *client)
{
    ...
    regulator_disable(d->nfc_regulator);
    regulator_put(d->nfc_regulator);
    i2c_set_clientdata(client, NULL);
...
static int __init pn547_dev_init(void)
{
    ...
    if (ret)
        pr_err("%s: SysClkReq4ClkBuf failed with errror %d\n", __func__, ret);
