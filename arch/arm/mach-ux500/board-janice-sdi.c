...
static int janice_wifi_cd; /* WIFI virtual 'card detect' status */ 
static unsigned int sdi1_card_status(struct device *dev)
{
	...
	if (sdi1_card_power_on) 
		...
		return 0;
#endif
	return janice_wifi_cd;
}
static void (*wifi_status_cb)(int card_present, void *dev_id);
static void *wifi_status_cb_devid;

static int janice_wifi_status_register(
...
#endif
	.register_status_notify = janice_wifi_status_register,
};
...
static struct resource brcm_wlan_resources[] = {
	[0] = {
		...
		.start	= GPIO_TO_IRQ(WL_HOST_WAKE_JANICE_R0_0),
		.end	= GPIO_TO_IRQ(WL_HOST_WAKE_JANICE_R0_0),
