...
static int bcm4330_bt_rfkill_set_power(void *data, bool blocked)
{
	...
	if (!blocked) {
		...
		msleep(20);
		gpio_set_value(BT_RST_N_GTI9060_R0_1, 1);
		msleep(50);
	} else {
...
static int bcm4330_bluetooth_probe(struct platform_device *pdev)
{
	...
	rc = gpio_request(BT_VREG_EN_GTI9060_R0_1, "bcm4330_bten_gpio");
	if (unlikely(rc)) {
		pr_err("[BT] BT_VREG_EN_GTI9060_R0_1 request failed.\n");
		return rc;
	}
	rc = gpio_request(BT_RST_N_GTI9060_R0_1, "bcm4330_btnrst_gpio");
	if (unlikely(rc)) {
		pr_err("[BT] BT_RST_N_GTI9060_R0_1 request failed.\n");
		gpio_free(BT_VREG_EN_GTI9060_R0_1);
		return rc;
	}
	rc = gpio_request(BT_WAKE_GTI9060_R0_1, "bcm4330_btwake_gpio");
	if (unlikely(rc)) {
	...
	rc = gpio_request(BT_HOST_WAKE_GTI9060_R0_1, "bcm4330_bthostwake_gpio");
	if (unlikely(rc)) {
